                ...
                else
                {
                    ...
                    if (exito == true && productoEscaneado.Any())
                    {
                        ...
                        foreach (SP_Escanear_CodigoResult unProductoEscaneado in productoEscaneado)
                        {
                            ...
                            // Verificar si el producto ya tiene una imagen en la base de datos
                            if (string.IsNullOrEmpty(unProductoEscaneado.Imagen))
                            {
                                // Buscar imagen del producto
                                string imagen = await new ImageService().BuscarImagenAsync(codigoBarras.Nombre, codigoBarras.Marca);

                                // Si se encontró una imagen, actualizarla en la base de datos
                                if (!string.IsNullOrEmpty(imagen))
                                {
                                    bool? exitoImagen = false;
                                    string mensajeImagen = "";
                                    miLinq.SP_ActualizarImagen(productoID, imagen, ref exitoImagen, ref mensajeImagen);

                                    if (exitoImagen == false)
                                    {
                                        res.mensaje.Add($"Error al actualizar la imagen: {mensajeImagen}");
                                    }
                                    else
                                    {
                            ...
                            {
                                codigoBarras.Imagen = unProductoEscaneado.Imagen;
                            }

                            codigoBarras.productoID = productoID;
